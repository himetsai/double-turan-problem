\documentclass[10pt, reqno]{report}
...
\begin{lemma}\label{lem:induce-reduce}
  ...
  \[
    \dex^*(m, n, F) = \frac{m}{k} \cdot \dex^*(k, n, F).
  \]
...
\begin{proof}
  Let $G_1, \ldots, G_m$ be induced doubly $F$-free graphs on $[n]$. Put $G_{i + m} = G_i$ for all $i \in [m]$. Then
  \[
  ...
  Now suppose $\sum_{i = 1}^m e(G_i) = \frac{m}{k} \cdot \dex^*(k, n, F)$ and $G_1 \neq G_2$. By assumption $\sum_{i = 1}^k e(G_i) < \dex^*(k, n, F)$. But then $\sum_{i = 1}^k e(G_{i + j}) > \dex^*(k, n, F)$ for some $j \geq 1$, contradiction. 
\end{proof}
...
\begin{proof}
  ...
  \]
  This establishes the upperbound. From this we also know that $e(G_1) + e(G_2) = \con(n, t, F)$ only if $G_1, G_2$ are the construction described by $\con(n, t, F)$. The result now follows.
\end{proof}
...
It turns out that $\dex(m, n, K_r)$ can be determined exactly, but the statement of the theorem requires some definitions. Let $k \geq 2$ and sets $S_{ij} \subseteq [m]$, for $1 \leq i < j \leq k$. We call the following type of construction a \textit{$k$-blowup}: Define $G_1, G_2, \ldots, G_m$ by partitioning $[n]$ into $k$ sets $V_1, V_2, \ldots, V_k$ and letting $\{u, v\} \in E(G_h)$ whenever $u \in V_i, v \in V_j, h \in S_{ij}$. Additionally, for each $i \in [k]$ and $\{u, v\} \subseteq V_i$, we place $\{u, v\}$ in exactly one of $G_1$. 
