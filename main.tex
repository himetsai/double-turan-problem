...
For a graph $F$, we say that $G_1, G_2, \dots, G_m$ is \textit{double $F$-free} if $E(F) \not \subseteq E(G_i) \cap E(G_j)$ for $1 \leq i < j \leq m$. In other words, $F$ is not contained in the intersection of any two of the graphs $G_i$. We call a copy of $F$ in the intersection of two of the graphs $G_i$ a \textit{double $F$}. The (non-induced) double Tur\'{a}n problem asks for the maximum value of $\sum_{i = 1}^m e(G_i)$ such that $G_1, G_2, \dots, G_m$ does not contain a double $F$. We denote these maxima by $\phi(m, n, F)$.

We would also like to study the case where the graphs $G_1, G_2, \ldots, G_m$ are \textit{induced}, meaning that every $G_i$ is an induced subgraph of $\bigcup_{i = 1}^m G_i$. In other words, if $\{u,v\} \in E(G_i)$ and $u,v \in V(G_j)$, then $\{u,v\} \in E(G_j)$. Let $\phi^*(m, n, F)$ denote the maximum value of $\sum_{i = 1}^m e(G_i)$ such that $G_1, G_2, \dots, G_m$ does not contain a double $F$ and $G_1, G_2,\dots, G_m$ are induced. The induced double Tur\'{a}n problem is then to determine $\phi^*(m, n, F)$ for various graphs $F$.

Clearly, $\phi(m, n, F) \geq \phi^*(m, n, F)$. We will see that the study of $\phi(m, n, F)$ and $\phi^*(m, n, F)$ is motivated by certain hypergraph extremal problems.
