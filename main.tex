...
\begin{lemma}\label{lem:induce-reduce}
  Let $n, m, k \geq 1$ such that $2 \leq k \leq m$, and let $F$ be some graph. Then
  \[
    \dex^*(n, m, F) \leq \frac{m}{k} \cdot \dex^*(n, k, F).
  \]
  Moreover, if the extremal condition for $k$ induced subgraphs is met only when $G_1 = \cdots = G_k$, then the above equality holds and the extremal condition for $m$ induced subgraphs is met if and only if $G_1 = \cdots = G_m$.
\end{lemma}
...
\begin{lemma}\label{lem:induce-cond}
  ...
  \]
  and the extremal condition is met if and only if $G_1 = \cdots = G_m$ are extremal graphs for $F$.
\end{lemma}
...
\begin{theorem}\label{thm:induce-complete-no-triangle}
  ...
  \]
  and the extremal condition is met if and only if $G_1 = \cdots = G_m = T_{r}(n)$.
\end{theorem}
...
\begin{theorem}\label{thm:induce-triangle}
  ...
  \]
  Moreover, if $n$ is even or $m$ is odd, then the extremal condition is met if and only if $G_1 = \cdots = G_m$ are complete balanced bipartite graphs on $n$ vertices. Otherwise, the extremal condition is met if and only if when either $G_1 = \cdots = G_m = K_{\frac{n + 1}{2}, \frac{n - 1}{2}}$ or $G_{2k + 1} = K_{\frac{n - 1}{2}, \frac{n - 1}{2}}$ and $G_{2k} = K_{\frac{n - 1}{2}, \frac{n - 1}{2}} + K_1$ for all $k \in [m/2]$.
\end{theorem}

\begin{proof}
  % By \cref{lem:induce-cond} and \cref{lem:induce-complete-cond}, we are done if $n$ is even, so we may assume that $n$ is odd. 
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      &= 2a + 1 > 0.
    \end{align*}
    This shows that $f$ has a maximum of $2\left\lfloor \frac{n^2}{4} \right\rfloor$ at $c = n - 1$ and $c = n$, which implies $\dex^*(n, 2, K_3) = 2\left\lfloor\frac{n^2}{4}\right\rfloor$. We are done if $c = n$, so assume that $c = n - 1$ and $a = 0$. Then $G_1 = G_{1, 2} = K_{\left\lfloor\frac{n - 1}{2}\right\rfloor, \left\lceil\frac{n - 1}{2}\right\rceil}$ and
    \[
...
Since $n$ cannot be both even and odd, we have the following corollary:

\begin{corollary}
  For $n \geq 1$,
  \[
    \dex^*(n, n, K_3) = n \left\lfloor \frac{n^2}{4} \right\rfloor,
  \]
  and the extremal condition is met if and only if $G_1 = \cdots = G_n$ are complete balanced bipartite graphs on $n$ vertices. 
\end{corollary}
...
\begin{theorem}\label{thm:induce-complete}
  For $n \geq 1$ and $m, r \geq 2$, 
  \[
    \dex^*(n, m, K_{r + 1}) = m \cdot \ex(n, K_{r + 1}).
  \]
  Moreover, the extremal condition is met if and only if $G_1 = \cdots = G_m = T_r(n)$, unless $r = 2$, $n$ is odd, and $m$ is even, in which case the extremal condition is met if and only if when either $G_1 = \cdots = G_m = K_{\frac{n + 1}{2}, \frac{n - 1}{2}}$ or $G_{2k - 1} = K_{\frac{n - 1}{2}, \frac{n - 1}{2}}$ and $G_{2k} = K_{\frac{n - 1}{2}, \frac{n - 1}{2}} + K_1$ for all $k \in [m/2]$.
\end{theorem}
...
\begin{theorem}
  For $n \geq 1$, and $m, r \geq 2$, if $F$ is $(r + 1)$-colorable, then large enough $n$
  \[
    \dex^*(n, m, F) = m \cdot \ex(n, F),
  \]
  and the extremal condition is met if and only if $G_1 = G_2 = \cdots = G_n$ are extremal graphs for $F$, unless $r = 2$, $n$ is odd, and $m$ is even, in which case the extremal condition is met if and only if when either $G_1 = \cdots = G_m = K_{\frac{n + 1}{2}, \frac{n - 1}{2}}$ or $G_{2k - 1} = K_{\frac{n - 1}{2}, \frac{n - 1}{2}}$ and $G_{2k} = K_{\frac{n - 1}{2}, \frac{n - 1}{2}} + K_1$ for all $k \in [m/2]$.
\end{theorem}

\begin{proof}
  It suffices to show for $m = 2$ by \cref{lem:induce-reduce}. Let $a, b, c$ and $f(a, b, c)$ be defined as in the proof of \cref{lem:induce-cond}. We first show that $f$ fails to meet the desired bound for small $c$.
  ...
  \[
    \text{ex}(n, F) - \text{ex}(n - 1, F) \geq \text{ex}(n, K_{r + 1}) - \text{ex}(n - 1, K_{r + 1}).
  \]
  Thus by \cref{lem:induce-cond} and \cref{lem:induce-complete-cond}, we are done for $r > 3$ or $n$ is even. For $r = 2$ and $n$ odd, the proof is follows the argument for the triangle case. \textcolor{red}{TODO: maybe write this out.}
\end{proof}
