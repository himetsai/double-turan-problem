...
\begin{frame}
  ...
  The non-bipartite case for $\phi(m, n, K_r)$ is not as simple.

  \vspace{1cm}

  \only<1>{
    Intuitively, we might guess
    \[
      \phi(m, n, K_r) = \binom{n}{2} + (m - 1)\ex(n, K_r)
    \]
  }
  ...
  \only<2>{
    Intuitively, we might guess
    \[
      \cancel{\phi(m, n, K_r) = \binom{n}{2} + (m - 1)\ex(n, K_r)}
    \]
  }
\end{frame}
...
\begin{frame}
  ...
  We need the idea of \alert{$(m, n, k)$-blowup} to state the result for $\phi(m, n, K_r)$.
